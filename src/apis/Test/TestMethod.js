...
async function CommunityTestListGet() {
  try {
    ...
    // console.log("boardalllist" + response.data);
...
async function BoardTypeRecommendRanking() {
  try {
    ...
    console.log('recomend' + response.data);
    return response.data;
  } catch (error) {
    console.log(error);
  }
}
async function Commentlist (boardId) {
  try {
    const response = await Instance.get(`/api/v1/commentlist/${boardId}`);
    console.log(response.data);
    return response.data;
  } catch (error) {
    console.log(error);
